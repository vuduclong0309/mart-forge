...
def scaffold(mart_dir: Path, mart_name: str, prefix: str) -> dict:
    ...
    for m in contract_metrics:
        if m["ads_column"] and m["ads_column"] not in FIXTURE_ADS_METRIC_COLUMNS:
            return {
                "success": False,
                "errors": [
                    f"Contract metric {m['id']} binds to ADS column '{m['ads_column']}' "
                    f"which is not produced by the fixture template. The framework fixture "
                    f"generates metric columns {sorted(FIXTURE_ADS_METRIC_COLUMNS)}. "
